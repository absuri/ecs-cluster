...
#### ECS
resource "aws_ecs_cluster" "hw-ecs" {
  name = "hw-ecs"
}
...
resource "aws_launch_configuration" "ecs" {
  ...
  user_data            = "#!/bin/bash\necho ECS_CLUSTER=${aws_ecs_cluster.hw-ecs.name} > /etc/ecs/ecs.config"
...
resource "aws_ecs_service" "hello-world" {
  name            = "hello-world"
  name            = "family"
  task_definition = "${aws_ecs_task_definition.hello-world.id}" 
  cluster         = "${aws_ecs_cluster.hw-ecs.id}"
  desired_count   = 2
}
